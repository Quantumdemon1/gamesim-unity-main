...
namespace Gamesim.Presentation
{
    ...
    public static class RelationshipWeb
    {
        ...
        private const float LegendHeight = 104f;
        ...
        public static RectTransform Build(
            ...
            Func<string, Texture> portrait, Action<string> select)
        {
            ...
            Graph(root, state, focus, scale, font, portrait, select);
            Column(root, state, focus, scale, font, portrait);
            return root;
        }

        private static float Radius(int count, float scale) =>
            Mathf.Min((112f + Mathf.Max(0, count - 6) * 6f) * scale, 150f);
            ...
            Func<string, Texture> portrait, Action<string> select)
        {
            ...
            Place(title.rectTransform, 4f * scale, -2f * scale, 300f * scale, 22f * scale);
            var strap = Text(area, StrapCopy, 12, UiTheme.Muted, UiTheme.Weight.Regular, scale, font);
            Place(strap.rectTransform, 4f * scale, -24f * scale, 330f * scale, 18f * scale);
            ...
            hub.anchoredPosition = new Vector2(0f, -(TitleHeight * scale + ringExtent));
            hub.sizeDelta = Vector2.zero;

            // Edges first, so the portraits draw over them.
            for (int i = 0; i < others.Count; i++)
                Edge(hub, Vector2.zero, Ring(i, others.Count, radius), KindOf(state, others[i].id),
                    state.Score(state.playerId, others[i].id), scale);

            string focusId = focus != null ? focus.id : null;
            for (int i = 0; i < others.Count; i++)
                Node(hub, state, others[i], Ring(i, others.Count, radius), NodeSize * scale, scale, font, portrait,
            ...
            Legend(area, legendTop, scale, font);
        }
        ...
        private static RectTransform NameChip(Transform parent, string word, Color tint, float scale, TMP_FontAsset font)
        {
            float width = Mathf.Max(40f, word.Length * 7.4f + 16f) * scale;
        ...
        private static void Legend(RectTransform area, float top, float scale, TMP_FontAsset font)
        {
            ...
            {
                float x = 6f * scale + (i % 3) * cell;
                float y = -(i / 3) * rowHeight;
            ...
            float markRow = -2f * rowHeight;
            for (int i = 0; i < marks.Length; i++)
            {
                float x = 6f * scale + i * cell;
                ...
                if (glyph != null)
                {
                    ...
                    Place(art, x + 4f * scale, markRow - 3f * scale, 13f * scale, 13f * scale);
                    var image = art.GetComponent<Image>();
                ...
                else
                {
                    ...
                    Place(pip, x + 5f * scale, markRow - 4f * scale, 10f * scale, 10f * scale);
                }
                var text = Text(legend, caption, 12, UiTheme.Muted, UiTheme.Weight.Regular, scale, font);
                Place(text.rectTransform, x + 28f * scale, markRow - 1f * scale, cell - 30f * scale, 18f * scale);
            }
            ...
            note.rectTransform.anchoredPosition = new Vector2(0f, -3f * rowHeight - 4f * scale);
            ...
            note.rectTransform.sizeDelta = new Vector2(note.rectTransform.sizeDelta.x, 36f * scale);
        }
            ...
            Func<string, Texture> portrait)
        {
            ...
            element.preferredWidth = ColumnWidth * scale; element.minWidth = ColumnWidth * scale;
            ...
            var glass = HudPrimitives.Glass("Glass", column);
            ...
            TMP_FontAsset font, Func<string, Texture> portrait)
        {
            ...
            Fit(name, 10f * scale);
            Place(name.rectTransform, left, -1f * scale, width, 24f * scale);
            ...
            Fit(who, 9f * scale);
            Place(who.rectTransform, left, -26f * scale, width, 18f * scale);
            ...
            x += Chip(row, mood, MoodColour(mood), x, -48f * scale, scale) + 6f * scale;
            ...
            if (badge != null) Chip(row, badge, badge == "NOM" ? UiTheme.Danger : UiTheme.Gold, x, -48f * scale, scale);
        }
