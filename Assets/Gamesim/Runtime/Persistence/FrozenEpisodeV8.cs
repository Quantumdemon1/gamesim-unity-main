...
{
    /// <summary>
    /// Recursive schema-8 persistence shape, frozen the moment schema 9 was cut.
    ///
    /// <para>Schema 8 is v7 plus eviction night's stages and speeches, the backdoor plan, the
    /// out-of-phase action count, the opening beats, and two more card fields. Its nested rows
    /// reference the already-frozen contracts rather than the runtime DTOs, which is the whole
    /// point: the runtime types keep growing and a frozen contract that followed them would
    /// validate nothing.</para>
    ///
    /// <para>The bounds are v7's. Schema 8 changed what a season records, not how much of anything
    /// it may hold: three to sixteen houseguests, every cast-derived collection bounded by sixteen,
    /// and the social-action counters at the flat allowance that was legal when v8 was written.</para>
    /// </summary>
    internal static class FrozenEpisodeV8
    {
...
#pragma warning restore 0649
        ...
        {
            if (original == null || original["schemaVersion"]?.Type != JTokenType.Integer || (long)original["schemaVersion"] != 8)
                throw new InvalidDataException("Expected simulation schema version 8.");
            SaveJson.CheckDtoShape(original, typeof(State), "state(v8)");
            try
            {
            ...
            {
                throw new InvalidDataException("Historical v8 data exceeds its frozen numeric or serialization contract.", error);
            }
        }

        /// <summary>
        /// The numeric and collection contract accepted v8 data had to satisfy. Do not widen it when
