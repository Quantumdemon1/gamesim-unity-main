...
{
    /// <summary>
    /// Recursive schema-11 persistence shape, frozen the moment schema 12 was cut.
    ///
    /// <para>Schema 11 is v10 plus two systems in one version: <c>boughtActionPoints</c> for the
    /// social vocabulary, and <c>houseEvents</c> with its <c>eventRulesStartWeek</c> boundary for
    /// the event layer.</para>
    ///
    /// <para>The bounds are v9's, and v9's were v7's. What a v9 save may <i>hold</i> never changed —
    /// three to sixteen houseguests, every cast-derived collection bounded by sixteen, and the
    /// social-action counters at the flat allowance that was still legal when v9 was written. That
    /// last one matters: a v9 save could carry eighteen spent actions, and a contract that used
    /// today's tighter ceiling would reject a save that was perfectly legal when written.</para>
    /// </summary>
    internal static class FrozenEpisodeV11
    {
...
#pragma warning disable 0649
        ...
        /// <summary>
        /// A v7 contestant: v2's row plus schema 7's card copy.
        ///
        /// <para>Spelled out rather than inherited from <see cref="FrozenEpisodeV2.Contestant"/>.
        /// The shape check compares an object against a type's declared fields, and a frozen row
        /// that reached for a shared base would start following whatever that base became.</para>
        /// </summary>
        internal sealed class Contestant
        {
            ...
            public int age;
            public string mood, stressLevel;
            public bool isPlayer;
            public int status;
            public FrozenEpisodeV2.Stats stats;
        ...
        internal sealed class State
        {
            ...
            public int evictionStage;
            public List<EvictionSpeech> evictionSpeeches;
            ...
            public List<HouseEvent> houseEvents;
            public int eventRulesStartWeek;
        }

        /// <summary>A house event as schema 11 stored it, frozen rather than following the runtime type.</summary>
        internal sealed class HouseEvent
        {
            public string id, kind, title, narrative, outcome;
            public List<string> involvedIds;
            public List<HouseEventOption> choices;
            public int week, chosenIndex;
            public bool resolved;
        }

        internal sealed class HouseEventOption
        {
            public string label, description, risk;
            public List<HouseEventEffect> impacts;
            public double trustChange;
        }

        internal sealed class HouseEventEffect
        {
        ...
        /// <summary>
        /// A deal as schema 10 stored it.
        ///
        /// <para>Frozen here rather than pointed at <see cref="Gamesim.Simulation.DealState"/>: the
        /// runtime type is free to grow, and a contract that followed it would start accepting
        /// shapes v10 never wrote.</para>
        /// </summary>
        internal sealed class Deal
        {
            public string id, type, proposerId, recipientId, targetId, status, trustImpact;
            public int week, expiresWeek;
        }

        internal sealed class EvictionSpeech
        {
...
#pragma warning restore 0649
        ...
        {
            if (original == null || original["schemaVersion"]?.Type != JTokenType.Integer || (long)original["schemaVersion"] != 11)
                throw new InvalidDataException("Expected simulation schema version 11.");
            SaveJson.CheckDtoShape(original, typeof(State), "state(v9)");
            try
            {
            ...
            {
                throw new InvalidDataException("Historical v9 data exceeds its frozen numeric or serialization contract.", error);
            }
        }

        /// <summary>
        /// The numeric and collection contract accepted v9 data had to satisfy. Do not widen it when
        ...
        internal static void ValidateScalarRanges(State s)
        {
            ...
            foreach (var c in cast)
            {
                ...
                Range(c.age, 0, 120, "age");   // Schema 7's own field, so v7 bounds it.
            ...
            // Schema 8's own fields, bounded at what v9 accepted.
            if (s.evictionStage < 0 || s.evictionStage > 4) throw Invalid("evictionStage");
            foreach (var speech in Rows(s.evictionSpeeches, 0, 2, "evictionSpeeches"))
                Range(speech.week, 1, s.week, "evictionSpeech.week");
            Range(s.outOfPhaseSocialActions, 0, 18, "outOfPhaseSocialActions");
            Rows(s.openingBeatsSeen, 0, 16, "openingBeatsSeen");

            // Schema 10's only addition. Bounded the way every other rules boundary is: a save may
            // declare next week but not an arbitrary week, so it cannot park the rule indefinitely.
            Range(s.socialBudgetRulesStartWeek, 1, Math.Min(101, s.week + 1), "socialBudgetRulesStartWeek");
            // Two hundred is what v10's own validation allowed, so it is what this accepts.
            foreach (var deal in Rows(s.deals, 0, 200, "deals"))
            {
                Range(deal.week, 1, s.week, "deal.week");
                Range(deal.expiresWeek, 0, 101, "deal.expiresWeek");
            }
            Range(s.dealRulesStartWeek, 1, Math.Min(101, s.week + 1), "dealRulesStartWeek");
            // What v11's own validation allowed, not what today's does.
            Range(s.boughtActionPoints, 0, 100, "boughtActionPoints");
            foreach (var item in Rows(s.houseEvents, 0, 400, "houseEvents"))
            {
                Range(item.week, 1, s.week, "houseEvent.week");
                Range(item.chosenIndex, -1, 8, "houseEvent.chosenIndex");
                foreach (var choice in Rows(item.choices, 0, 8, "houseEvent.choices"))
                {
                    Range(choice.trustChange, -100, 100, "houseEvent.trustChange");
                    foreach (var impact in Rows(choice.impacts, 0, 32, "houseEvent.impacts"))
                        Range(impact.amount, -100, 100, "houseEvent.impact.amount");
                }
            }
            Range(s.eventRulesStartWeek, 1, Math.Min(101, s.week + 1), "eventRulesStartWeek");
        ...
        private static InvalidDataException Invalid(string field)
            => new InvalidDataException("Historical v9 scalar/collection contract is invalid: " + field + ".");
    }
}
