...
{
    /// <summary>
    /// Recursive schema-5 persistence shape. Shared older nested shapes reference only
    /// the already-frozen v2 contract, never the growing runtime simulation DTOs.
    /// </summary>
    internal static class FrozenEpisodeV5
    {
...
#pragma warning restore 0649
        ...
        {
            if (original == null || original["schemaVersion"]?.Type != JTokenType.Integer || (long)original["schemaVersion"] != 5)
                throw new InvalidDataException("Expected simulation schema version 5.");
            SaveJson.CheckDtoShape(original, typeof(State), "state(v5)");
            try
            {
                ValidateScalarRanges(original.ToObject<State>(SaveJson.Serializer()));
                // Only frozen v5 guards run here. New fields/defaults on the DTO are not inspected.
                FrozenV5Validator.Validate(original.ToObject<EpisodeState>(SaveJson.Serializer()));
            }
            catch (Exception error) when (error is JsonException || error is OverflowException)
            {
                throw new InvalidDataException("Historical v5 data exceeds its frozen numeric or serialization contract.", error);
            }
        }

        // Explicit numeric/collection contract from accepted v5. Do not widen when later runtime rules grow.
        ...
        private static InvalidDataException Invalid(string field) => new InvalidDataException("Historical v5 scalar/collection contract is invalid: " + field + ".");
