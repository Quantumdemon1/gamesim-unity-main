...
| **Phase 4** — duration easing, close-range name tags, ceremony framing presets | **Done 2026-09-19.** `HouseCameraRig.MoveTo(focus, distance, seconds)` eases a scripted move over exactly its duration (smoothstep on the clock; any input ends it where it is; reduced motion lands at once), for beats that have to land on a count. A houseguest's name tag fades with the camera's distance — full inside 9 m, gone past 14 m — so a conversation shows who is talking and a wide shot is not a screen of labels (`HouseNpc.NameTagAlpha`). Ceremony framing presets: when a ceremony's card plays the director sends the camera to its room on the clock — the nomination room, the game room for the veto, the living room for an eviction and the winner, the yard for a competition — holds through the card and the reveal, and comes back (`EpisodeDirector.CeremonyFraming`; reduced motion keeps the shot). `Camera_ATimedMove…`, `Camera_ACeremony…` and `NameTags_…` PlayMode tests |
