...
{
    /// <summary>
    /// Recursive schema-7 persistence shape, frozen the moment schema 8 was cut.
    ///
    /// <para>Schema 7 is v6 plus the contestant card copy — occupation, archetype and age. Its
    /// nested rows reference the already-frozen v2 and v5 contracts rather than the runtime
    /// simulation DTOs, which is the whole point: the runtime types keep growing and a frozen
    /// contract that followed them would validate nothing.</para>
    ///
    /// <para>The bounds are v6's, because schema 7 changed the contestant's fields and nothing
    /// about how many of anything a season may hold: three to sixteen houseguests, and every
    /// cast-derived collection bounded by sixteen.</para>
    /// </summary>
    internal static class FrozenEpisodeV7
    {
...
#pragma warning restore 0649
        ...
        {
            if (original == null || original["schemaVersion"]?.Type != JTokenType.Integer || (long)original["schemaVersion"] != 7)
                throw new InvalidDataException("Expected simulation schema version 7.");
            SaveJson.CheckDtoShape(original, typeof(State), "state(v7)");
            try
            {
            ...
            {
                throw new InvalidDataException("Historical v7 data exceeds its frozen numeric or serialization contract.", error);
            }
        }

        /// <summary>
        /// The numeric and collection contract accepted v7 data had to satisfy. Do not widen it when
        ...
        private static InvalidDataException Invalid(string field)
            => new InvalidDataException("Historical v7 scalar/collection contract is invalid: " + field + ".");
    }
}
